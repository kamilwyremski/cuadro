...
if(!empty($_POST['base_url']) and !empty($_POST['server']) and !empty($_POST['user']) and !empty($_POST['name']) and !empty($_POST['admin']) and !empty($_POST['password_admin']) and !empty($_POST['password_admin_repeat']) and !empty($_POST['email']) and isset($_POST['db_prefix']) and isset($_POST['pass_hash'])){
...
');
			...
			$sql = file_get_contents('cuadro.sql');
...
<body>
<div class="container">
	<a href="http://wyremski.pl" title="Creating websites"><img src="../admin/images/admin.png" alt="Admin Panel created by Kamil Wyremski" id="logo"/></a>
	...
	<form method="post" class="form-horizontal">
		...
		<div class="form-group">
			<label class="col-sm-5 control-label"><?= lang('Salt passwords in the system') ?>:</label>
			<div class="col-sm-7">
				<input class="form-control" type="text" name="pass_hash" placeholder="<?= lang('Salt passwords in the system') ?>" value="<?php if(isset($_POST['pass_hash'])){echo($_POST['pass_hash']);}?>" title="<?= lang('Salt passwords in the system') ?>"/>
			</div>
		</div>
