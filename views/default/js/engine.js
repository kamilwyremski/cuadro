$(document).ready(function () {
  $("#form_add_file").submit(function () {
    const $this = $(this);
    $this.find("[type=submit]").attr("disabled", true);
    $this.find("[type=submit] .material-icons").show();
  });
    ...
    .click(function () {
      ...
      $.ajax({
        ...
        success: function (response) {
          if (
            typeof response.plus !== "undefined" &&
            typeof response.minus !== "undefined"
          ) {
            $this
              .parents(".set_voice_outside")
              .find(".voice_plus")
              .text(response.plus);
            $this
              .parents(".set_voice_outside")
...
$(window).on("load", function () {
  ...
  if ($js_scroll_page.length > 0) {
    ...
    if ($(window).scrollTop() + $(window).height() < position) {
      $("html, body")
        .stop()
        .animate({ scrollTop: position - 110 }, 300);
    }
  }
});
