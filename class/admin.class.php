...
class admin {
	...
	public function login($data){
		...
		if($sth->rowCount()<5){
			...
			if($user_data){
				...
				return ['status'=>false,'error'=>lang('The entered data are incorrect')];
			}
		}else{
			return ['status'=>false,'error'=>lang('Exceeded the limit login attempts')];
		}
	}
	...
	public function changeUser($data){
		...
		if($data['new_password']==$data['repeat_new_password']){
			if($data['new_username']!=$this->user_data['username']){
				...
				$sth->execute();
				if($sth->fetchColumn()){
					return ['status'=>false,'error'=>lang('The selected username is already taken')];
				}
			}
			...
			return ['status'=>true,'info'=>lang('The data have been updated')];
		}else{
	...
	public function removeLogs(){
		...
		return ['status'=>true,'info'=>lang('Logs logon to the Admin Panel has been successfully removed')];
	}
	...
	public function addUser($data){
		...
		if($data['password']==$data['repeat_password']){
			...
			if(!$sth->fetchColumn()){
				...
				$sth->bindValue(':password', $this->createPassword($data['password']), PDO::PARAM_STR);
				$sth->execute();
				return ['status'=>true,'info'=>lang('Added new user')];
			}else{
				return ['status'=>false,'error'=>lang('The selected username is already taken')];
			}
		}else{
	...
	public function removeUser($id){
		...
		if($id!=$this->user_data['id']){
			...
			$sth->execute();
			return ['status'=>true,'info'=>lang('User has been successfully removed')];
		}else{
			return ['status'=>false,'error'=>lang('You can not delete a user who is logged')];
		}
	}
